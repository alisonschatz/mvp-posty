...
import { ThumbsUp, MessageCircle, Share, Users, MoreHorizontal, Camera, Edit3 } from 'lucide-react';

const FacebookPreview = ({ content, image, isEditing, onContentChange, onImageEdit, onImageUpload }) => {
  ...
  return (
    <div className="max-w-md mx-auto bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
      <div className="flex items-center p-4">
        <div className="w-10 h-10 bg-blue-600 rounded-full flex items-center justify-center text-white font-medium">SP</div>
        <div className="ml-3 flex-1">
          <p className="font-medium text-gray-900">Seu Perfil</p>
          <div className="flex items-center text-xs text-gray-500">
            <span>2 min</span><span className="mx-1">•</span><Users className="w-3 h-3 mr-1" />
          </div>
        </div>
      ...
      <div className="border-t border-gray-100">
        <div className="flex items-center justify-between px-4 py-2 text-sm text-gray-500">
          <span>{likes} curtidas</span><span>12 comentários</span>
        </div>
