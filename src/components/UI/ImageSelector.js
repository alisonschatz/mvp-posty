...
const ImageSelector = ({ 
  ...
  // Função para carregar fotos sugeridas
  const loadSuggestedPhotos = useCallback(async () => {
    setPhotosLoading(true);
    try {
      console.log('Buscando fotos inteligentes...');
  ...
  const generateAI = useCallback(async () => {
    ...
    try {
      ...
      console.log('Imagens IA geradas:', result?.images?.length || 0);
    } catch (error) {
  ...
  }, [generatedContent, conversationData]);

  // Selecionar imagem
  const selectImage = useCallback(async (image) => {
    setSelectedImage(image);
    try {
      // Para imagens do DALL-E, usar URL diretamente (evitar CORS)
      if (image.source === 'dalle') {
        console.log('DALL-E image selected, using URL directly');
        onImageSelect(image.urls.regular || image.urls.small);
        return;
      }

      // Para outras fontes, tentar converter para base64
  ...
  // Verificar se está usando descrição inteligente
  const isUsingSmartDescription = generatedContent && typeof generatedContent === 'string';
      ...
      <div className="mb-6">
        <div className="flex gap-3 mb-4">
          ...
          <button
            ...
            className="bg-orange-500 text-white px-6 py-3 rounded-xl hover:bg-orange-600 disabled:opacity-50 transition-colors"
          >
          ...
          <button
            ...
            className="bg-gray-500 text-white px-4 py-3 rounded-xl hover:bg-gray-600 disabled:opacity-50 transition-colors"
        ...
        <div className="flex gap-2">
          {['all', 'pexels', 'unsplash'].map((source) => (
            <button
              key={source}
              onClick={() => setActiveSource(source)}
              className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${
                activeSource === source
                  ? source === 'all' ? 'bg-orange-500 text-white' 
                    : source === 'pexels' ? 'bg-green-500 text-white'
      ...
      <div className="bg-gradient-to-r from-purple-50 to-pink-50 rounded-xl p-6 mb-6">
        <div className="text-center">
          ...
          <button
            onClick={generateAI}
            disabled={aiLoading}
            className="bg-purple-600 text-white px-8 py-3 rounded-xl hover:bg-purple-700 disabled:opacity-50 flex items-center gap-2 mx-auto transition-colors"
          >
            {aiLoading ? (
              <>
  ...
  // Se está embarcado, renderizar apenas o conteúdo interno
  if (isEmbedded) {
    return (
  ...
  // Renderização completa como modal
  return (
